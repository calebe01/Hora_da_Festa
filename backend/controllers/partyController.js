    ...
    create: async (req, res) => {
        try {
            ...
            if (party.services && !checkPartyBudget(party.budget, party.services)) {
                res.status(406).json({ msg: "sem cash nao entra" });
                return;
            }

            const createdParty = await PartyModel.create(party);
                       

            res.status(201).json({ createdParty, msg: "Hoje tem festa la no meu AP" });
        } catch (error) {
            console.log(error);
            res.status(500).json({ error: "Ocorreu um erro ao criar a festa" });
        }
    },
